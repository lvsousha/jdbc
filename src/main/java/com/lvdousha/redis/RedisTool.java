...
public class RedisTool {
	...
	private static String address = "localhost";			// Redis服务器地址
	private static int port = 6379;							// Redis服务器端口号
	private static String auth = null;						// 访问密码
	// 可用连接实例的最大数目，默认值为8；
	// 如果赋值为-1，则表示不限制；如果pool已经分配了maxActive个jedis实例，则此时pool的状态为exhausted(耗尽)。
	// private static final int MAX_ACTIVE = 1024;

	private static int MAX_IDLE = 10;						// 控制一个pool最多有多少个状态为idle(空闲的)的jedis实例，默认值也是8。
